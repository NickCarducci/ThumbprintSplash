...
class USBudget extends React.Component {
                ...
                fetch(bondURL)
                  ...
                  .then((result) => {
                    ...
                    result.data.map((data) => {
                      ...
                      if (!isNaN(bondNewValue) && !isNaN(bondRedemptionRatio)) {
                        bondDates.push(bondDate);
                        bondNewValues.push(bondNewValue);
                        bondNets.push(bondRedemptionRatio);
                        bonds.push({
                          bondDate,
                          bondNewValue
                        });
  ...
  render() {
    ...
    const intExp = intexp.map((x, i) => [
        ((x.intDate - lowestDates) / (highestDates - lowestDates)) *
          (this.props.width - 30),
        ((x.intValue - lowestValues) / (highestValues - lowestValues)) * 260
      ]),
      gifting = gifts.map((x, i) => [
        ((x.giftDate - lowestDates) / (highestDates - lowestDates)) *
          (this.props.width - 30),
        ((x.giftValue - lowestValues) / (highestValues - lowestValues)) * 260
      ]),
      outlayed = outlays.map((x, i) => [
        ((x.outDate - lowestDates) / (highestDates - lowestDates)) *
          (this.props.width - 30),
        ((x.recValue - lowestValues) / (highestValues - lowestValues)) * 260
      ]),
      outlayer = outlays.map((x, i) => [
        ((x.outDate - lowestDates) / (highestDates - lowestDates)) *
          (this.props.width - 30),
        ((x.outValue - lowestValues) / (highestValues - lowestValues)) * 260
      ]),
      bonding = bonds.map((x, i) => [
        ((x.bondDate - lowestDates) / (highestDates - lowestDates)) *
          (this.props.width - 30),
        ((x.bondNewValue - lowestValues) / (highestValues - lowestValues)) * 260
      ]);
    var lowestBondNetValues = Math.min(...bondNets);
    var highestBondNetValues = Math.max(...bondNets);
    const netting = bondsNetting.map((x, i) => [
      ((x.bondDate - lowestDates) / (highestDates - lowestDates)) *
        (this.props.width - 30),
